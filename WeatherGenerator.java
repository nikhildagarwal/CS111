...
public class WeatherGenerator {
    ...
    int month, double[][] drywet, double[][] wetwet) {
        ...
        for (int ir = 0; ir < 4001; ir++) {
            if (longitute == wetwet[ir][0] && latitude==wetwet[ir][1]){
                rowWet = ir;
            }
        }
        int[] monthForecast = new int[numberOfDaysInMonth[month - 2]];
        ...
        for (int g = 1; g < numberOfDaysInMonth[month - 2]; g++) {
            prob = StdRandom.uniform();
            if (monthForecast[g - 1] == WET) {
                if (prob <= wetwet[rowWet][month]) {
                    monthForecast[g] = WET;
                } else {
                    monthForecast[g] = DRY;
                }
            } else {
                if (prob <= drywet[rowDry][month]) {
                    monthForecast[g] = WET;
                } else {
                    monthForecast[g] = DRY;
                }
    ...
    public static int lengthOfLongestSpell (int[] forecast, int mode) {
        int largestValue = 0;
        int countValue = 0;
        boolean previousEquals = false;
        for(int e = 0; e<forecast.length; e++){
            if(forecast[e] == mode && previousEquals == false){
                countValue=1;
                previousEquals = true;
            }else{
                if(forecast[e] == mode && previousEquals == true){
                    countValue++;
                    if(countValue > largestValue){
                        largestValue = countValue;
                    }
                } else{
                    if(forecast[e] != mode){
                        countValue = 0;
